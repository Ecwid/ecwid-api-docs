...
product_details_gallery_layout | string | Set different **layout of gallery on product pages**. <ul><li>`"IMAGE_SINGLE_THUMBNAILS_HORIZONTAL"` shows them below image in a classic way with small thumbnails next to each other.</li><li>`"IMAGE_SINGLE_THUMBNAILS_VERTICAL"` shows additional product images on the left from the main image.</li><li>`"IMAGE_FEED"` shows the them as feed with large detaied images.</li></ul>Defalut value: `"IMAGE_SINGLE_THUMBNAILS_HORIZONTAL"`
